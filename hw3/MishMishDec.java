										...
										if ((stackindex > 1) && (n.history.size() >= 2)) {
											...
											for (Node stacknode : stacks.get(stackindex)) {
												if (stacknode.history.size() >= 2) {
													...
													String stackhist = stacknode.history.get(stacknode.history.size() - 2) + " " + stacknode.history.get(stacknode.history.size() - 1);
													if (n.history.size() < 2)
														System.out.println("hist < 2: " + n.historystring());
													String hist = n.history.get(n.history.size() - 2) + " " + n.history.get(n.history.size() - 1);
													String ncov = "";
													String sncov = "";
													...
													if (stackhist.equals(hist) && ncov.equals(sncov) && stacknode.lastsp.equals(n.lastsp)) {
														...
														if ((stacknode.score) < (n.score)) {
															removeindex = stack.get(stackindex).indexOf(stacknode);

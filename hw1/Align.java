		...
		while(alignfile.hasNextLine()){
			...
			while(atok.hasMoreTokens()){ 
				...
				if(dash != -1){ //skip if possible alignment
					...
					//System.out.println("jprime = " + j1 + " jw = " + (j - j1));
				...
				for (int eindex = 0; eindex < enlist.size(); eindex++) {
					...
					for (int dindex = 0; dindex < delist.size(); dindex++) {
						...
						if(jprime == -1)
							...
							String key = dindex + " " + jprime + " " + delist.size();
							try{
								prob = (0.75*efprob.get(ed)) * (0.25*transprob.get(key)); 
							}
							catch(NullPointerException npe){
								System.out.println("NullPointer Exception");
								System.out.println("e: " + eindex + " " + e);
								System.out.println("\tfindex: " + dindex + " " + d);
								System.out.println("\tmaxD = " + maxD);
								System.out.println("\tkey = " + key + " tp = " + transprob.get(key));
								System.out.println("\ted = " + ed + " efprob = " + efprob.get(ed));
								System.out.println("jprime = " + jprime);
								System.out.println("\tprob = " + prob);
							}
						}

						//aligntable[eindex][dindex] = prob; //save the alignment prob
